...
def dir_to_spectrogram(audio_dir, spectrogram_dir, spectrogram_dimensions=(64, 64), noverlap=16, cmap='gray_r'):
    ...
    :return:
    """
    file_names = [f for f in listdir(audio_dir) if isfile(join(audio_dir, f)) and '.wav' in f] # + [f for f in listdir(audio_dir) if isfile(join(audio_dir, f)) and '.mp3' in f]  (https://github.com/scipy/scipy/issues/3536)
...
if __name__ == '__main__':
    audio_dir = str(Path.cwd()) + '\\'
    spectrogram_dir = str(Path.cwd()) + '\\'
